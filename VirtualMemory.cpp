...
word_t dfs_rec(word_t cur_frame, word_t prev_frame, uint64_t cur_page, uint64_t current_layer,
               ...
               word_t *case_3_parent_frame, uint64_t *case_3_offset, uint64_t offst, word_t found_frame,
               bool *child_is_empty, vaddress page_swapped_in) {
    ...
    *max_frame = (cur_frame > *max_frame) ? cur_frame : *max_frame;
    ...
    if (current_layer == TABLES_DEPTH) {
        uint64_t abs_page_diff = (page_swapped_in > cur_page) ? page_swapped_in - cur_page : cur_page -
                                                                                               page_swapped_in;
        ...
        if (dist_from_page > *max_cyclical_distance) {
            ...
            *page_with_max_cyclical_distance = cur_page;
            *frame_with_max_cyclical_distance = cur_frame;
    ...
    for (uint64_t offset = 0; offset < PAGE_SIZE; ++offset) {
        ...
        if (next_frame != 0) {
            ...
            // dfs recursion call:
            word_t dfs_address = dfs_rec(next_frame, cur_frame, (cur_page << OFFSET_WIDTH)
                                                                + offset, current_layer + 1, skip_this_frame,
                                         max_frame, page_with_max_cyclical_distance, frame_with_max_cyclical_distance,
                                         max_cyclical_distance, case_3_parent_frame, case_3_offset, offset, found_frame,
                                         child_is_empty, page_swapped_in);
...
word_t dfs(word_t skip_this_frame, word_t *max_frame, vaddress *page_with_max_cyclical_distance,
           word_t *frame_with_max_cyclical_distance, word_t *case_3_parent_frame, uint64_t *case_3_offset,
           vaddress page_swapped_in) {
    uint64_t max_c_d = 0;
    ...
    return dfs_rec(0, 0, 0, 0, skip_this_frame, max_frame,
                   ...
                   &max_c_d, case_3_parent_frame, case_3_offset, 0, 0,
...
word_t grab_frame(word_t skip_this_frame, vaddress page_swapped_in) {
    ...
    uint64_t case_3_offset = 0;
    word_t chosen_frame = dfs(skip_this_frame, &max_frame,
                              &page_with_max_cyclical_distance,
                              &frame_with_max_cyclical_distance,
                              &case_3_parent_frame, &case_3_offset, page_swapped_in);
    ...
    switch (get_priority_of_frame(chosen_frame, max_frame)) {
        ...
        case 3:
            ...
            PMwrite(case_3_parent_frame * PAGE_SIZE + case_3_offset, 0);
...
word_t get_Paddress(vaddress Vaddress) {
    ...
    for (uint64_t offsets = 0; offsets < TABLES_DEPTH; offsets++) {
        ...
        if (cur_padd == 0) {
            ...
            word_t frame = grab_frame(last_padd, Vaddress >> OFFSET_WIDTH);

            if (offsets + 1 == TABLES_DEPTH) {
                // then we reached the leaves
                PMrestore(frame, Vaddress >> OFFSET_WIDTH);
            } else {
                // zero out the table
                clear_frame(frame);
            }
        ...
        last_padd = cur_padd;
//        std::cout << "########## offset " << offsets << "##########" << std::endl;
//        printRam();
//        std::cout << "########## end of offset " << offsets << "##########" << std::endl;
